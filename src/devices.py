...
class CLIENT:
    ...
    # def eval_unlabel(self, net, dataset):
    #     if len(self.U_idxs) == 0:
    #         print('No unlabeled data.')
    #         return None
    #
    #     m = self.args.method
    #     # data_loader = DatasetSplit(dataset, self.U_idxs, self.relabel)
    #     data_loader = DataLoader(DatasetSplit(dataset, self.U_idxs, self.relabel), batch_size=1, shuffle=False)
    #     net.load_state_dict(self.model)
    #     net.to(self.args.device)
    #     net.eval()
    ...
    #
    #     with torch.no_grad():
    #         for idx, (images, labels) in enumerate(data_loader): # idx is the location of now unlabeled data
    #             # images, labels = images.to(self.args.device), labels.to(self.args.device)
    #             images = images.to(self.args.device)
    #
    #             y_, h, x_ = net(images)
    #             # local_labels = self.encoder(y_)
    #             # prob_dist = F.softmax(local_labels,dim=-1) # using local model
    #             prob_dist = F.softmax(y_, dim=-1)
    #             prob_dist = torch.squeeze(prob_dist)
    #             if m == 0:
    #                 score = least_confidence(prob_dist, sorted=False)
    #             elif m == 1:
    #                 score = margin_confidence(prob_dist)
    #             elif m == 2:
    #                 score = ratio_confidence(prob_dist)
    #             elif m == 3:
    #                 score = entropy_based(prob_dist)
    #
    #             samples_score.append(score)
    #         # idx ==> uncertainty score
    #         samples_score = np.array(samples_score)
    #         index = samples_score.argsort()[::-1]
    #         num = min(self.tag_num, len(index))
    #         tag_index = index[:num]
    #
    #     data_index = self.U_idxs[tag_index] # have been labeled
    #
    #     # update model with new label data
    #     newdata = DataLoader(DatasetSplit(dataset, data_index, self.relabel), batch_size=self.args.local_bs, shuffle=True)
    #
    #     net.train()
    #     self.encoder.train()
    ...
    #         for batch_idx, (images, labels) in enumerate(newdata):
    #             images, labels = images.to(self.args.device), labels.to(self.args.device)
    #             optimizer.zero_grad()
    #             y_, h, x_ = net(images)
    #             optimizer_enc.zero_grad()
    #             local_labels = self.encoder(y_)
    #             loss_enc = torch.sum(L_c(local_labels, labels))
    #             loss_enc.backward(retain_graph=True)
    #             optimizer_enc.step()
    #
    #             global_labels = torch.Tensor(np.array(self.global_embedding.find(h.data.cpu())[1])).long().to(
    #                 self.args.device)
    #             loss = torch.sum(L_c(y_, global_labels))
    #             loss.backward()
    #
    ...
    #
    #     ## update dataset:
    #     self.L_idxs = np.append(self.L_idxs, data_index)
    #     self.U_idxs = np.setdiff1d(self.U_idxs, data_index)
    #     self.Ldata = DataLoader(DatasetSplit(dataset, self.L_idxs, self.relabel), batch_size=self.args.local_bs, shuffle=True)
    #     if len(self.U_idxs):
    #         self.Udata = DataLoader(DatasetSplit(dataset, self.U_idxs, self.relabel), batch_size=self.args.local_bs,
    #                                 shuffle=True)
    #
